...
public class AuthEndpoint extends AbstractBaseEndpoint {
    ...
    @POST
    @Path("/token")
    public Response getAccessToken(TokenRequest tokenRequest) throws EntityException, ForbiddenException, MalformedClaimException
    ...
    @GET
    @Path("/token")
    public Response getTokenByRefreshToken(@QueryParam("refresh_token") String refreshToken) throws EntityException, ForbiddenException, MalformedClaimException

    {
        ApiResponse response = new ApiResponse();

        Token token =  AuthenticationService.getInstance().getTokenByRefreshToken(refreshToken);
